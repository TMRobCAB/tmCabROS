...
#define COM_CMD_SUCCESS         	(-11)
#define COM_CMD_NOT_SYNCHED			(-12)
#define COM_CMD_SYNCHED				(-13)
#define COM_CMD_REF_UDATED			(-14)
#define COM_CMD_SAFE_UDATED			(-15)

#define COM_CMD_ERR_SYNCH			(-16)
...
#define FSM_USLEEP_AFTER_SYNCH 1000000
...
class ComCmds {
public:
	ComCmds(ComPort * comPort);
	~ComCmds(){};

	int32_t CmdLineRead(SysFSM * sysFSM);
	int32_t CmdLineWrite(void);

	void AddCmds(unsigned char * cmdIDs, float * cmdArgs, uint8_t CmdNum);
	void AddCmd(unsigned char cmdIDs, float cmdArgs);

	int SendData(char * bData, uint dLen);
	bool Synch(void);
...
private:
	...
	unsigned char _cInput[APP_INPUT_BUF_SIZE];
	...
	int SendCmd(const unsigned char * cmdIDs, float * cmdArgs,
			uint32_t cmdsNum);

	int F_Ping(int8_t cmd, float arg);
};
